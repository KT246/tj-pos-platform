...
type TerminalToast = SystemToastMessage & {
  durationMs: number;
};

type TerminalToastInput = {
...
type TerminalToastState = {
  toasts: TerminalToast[];
  addToast: (toast: TerminalToastInput) => void;
...
function createToastId() {
  toastSequence += 1;
  return `terminal-toast-${Date.now()}-${toastSequence}`;
}
...
export const useTerminalToastStore = create<TerminalToastState>((set) => ({
  toasts: [],
  addToast: (toast) =>
    set((state) => ({
      toasts: [
...
export function pushTerminalToast(toast: TerminalToastInput) {
  useTerminalToastStore.getState().addToast(toast);
}

export function showTerminalNotice(
  message: string,
...
export function TerminalToastHost() {
  const toasts = useTerminalToastStore((state) => state.toasts);
  const removeToast = useTerminalToastStore((state) => state.removeToast);
