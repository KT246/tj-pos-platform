...
```text
ไม่สร้าง route Wholesale แยก
Orders ใช้ filter `orderType=retail|wholesale|purchase|return`
Wholesale order detail ต้องแสดง customer/reseller, price list, payment status, debt status, delivery status, quantity discount และ notes
```
...
หมายเหตุ Wholesale / Debt:

```text
ไม่สร้าง route Debt แยก
Payments ต้องรองรับสถานะ paid, partially paid, unpaid และ debt
ใช้หน้าจอ Payments เดิมเพื่อดู/บันทึกการชำระเงินเต็มจำนวน, ชำระบางส่วน, บันทึกเป็นหนี้ค้างชำระ และรับชำระหนี้ภายหลัง
```
...
Item เดียวกันต้องรองรับ default retail price, wholesale price, reseller price, min wholesale quantity และ special price by price list
```
...
ใช้จัดการ customer profile, member profile, wholesale customer และ reseller
...
```text
ขายปกติอาจไม่จำเป็นต้องมี customer profile
แต่ถ้าเปิด loyalty / membership / wholesale / debt ต้องใช้ customer profile
Customers ต้องรองรับ customer type: retail customer, wholesale customer, reseller, VIP
```
...
```text
Reports ใช้ filter เดิมและเพิ่ม customerType/orderType สำหรับมุมมอง:
Sales by customer type, wholesale sales, top wholesale customers, debt report, payment by bank, customer balance, profit by price type
```
...
```text
Import / Export ต้องรองรับ wholesale price, customer type, customer debt opening balance, customer debt export, wholesale price list export และ wholesale orders export
```
