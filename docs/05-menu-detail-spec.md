...
| reseller_price | ไม่บังคับ | ราคาสำหรับ reseller |
| min_wholesale_quantity | ไม่บังคับ | จำนวนขั้นต่ำเพื่อใช้ราคาขายส่ง |
| price_list | ไม่บังคับ | price list พิเศษตามกลุ่มลูกค้า |
...
| order_type | บังคับ | retail / wholesale / purchase / return / dine_in / takeaway / service / room |
| customer_id | ไม่บังคับ | ถ้ามี loyalty / wholesale / debt |
| price_list_id | ไม่บังคับ | ใช้กับลูกค้าส่งหรือ reseller |
| debt_status | ไม่บังคับ | none / unpaid / partial / debt / settled |
| delivery_status | ไม่บังคับ | pending / delivered / cancelled |
...
| Customer debt payments | ไม่ต้อง | ได้ | PDF ถ้าจำเป็น |
...
แต่ต้องมี customer profile เมื่อเปิด Loyalty / Membership / Wholesale / Debt

Wholesale ไม่ใช่ module แยกและไม่ใช่ TJ Wholesale
ให้ใช้ Customers เดิมเพื่อจัดการ retail customer, wholesale customer, reseller และ VIP
...
| customer_type | บังคับ | retail_customer / wholesale_customer / reseller / vip |
| price_list_id | ไม่บังคับ | price list ที่กำหนดให้ลูกค้า |
| debt_balance | ไม่บังคับ | ยอดหนี้คงค้าง |
| credit_limit | ไม่บังคับ | วงเงินเครดิต |
| payment_term | ไม่บังคับ | เงื่อนไขชำระเงิน เช่น due in 7/15/30 days |
| phone | บังคับถ้าเปิด loyalty/wholesale/debt | ใช้ค้นหา member/customer |
| email | ไม่บังคับ |  |
...
| Customer type | ได้ | ได้ | ไม่ต้อง |
| Customer debt opening balance | ได้ | ได้ | PDF ถ้าจำเป็น |
| Wholesale order history | ไม่ต้อง | ได้ | PDF ถ้าจำเป็น |
